...
class CFileValidator extends CValidator {
    ...
    protected function validateAttribute($object, $attribute) {
        if ($this->maxFiles > 1) {
            ...
            if (count($files) > $this->maxFiles) {
                $message = $this->tooMany !== null ? $this->tooMany :
    ...
    protected function validateFile($object, $attribute, $file) {
        ...
        elseif ($error == UPLOAD_ERR_INI_SIZE || $error == UPLOAD_ERR_FORM_SIZE
            || $this->maxSize !== null && $file->getSize() > $this->maxSize
        ) {
            ...
            throw new CException(Yii::t('yii', 'The file "{file}" was only partially uploaded.',
                    array ('{file}' => $file->getName())));
        ...
        elseif ($error == UPLOAD_ERR_CANT_WRITE)
            throw new CException(Yii::t('yii',
                    ...
                    array ('{file}' => $file->getName())));
        elseif (defined('UPLOAD_ERR_EXTENSION')
        ...
        if ($this->mimeTypes !== null) {
            if (function_exists('finfo_open')) {
                $mimeType = false;
                if ($info =
                    finfo_open(defined('FILEINFO_MIME_TYPE') ? FILEINFO_MIME_TYPE : FILEINFO_MIME)
                )
            ...
            if (is_string($this->mimeTypes))
                $mimeTypes =
                    preg_split('/[\s,]+/', strtolower($this->mimeTypes), -1, PREG_SPLIT_NO_EMPTY);
            else
