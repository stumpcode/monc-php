...
class CTypeValidator extends CValidator {
    ...
    protected function validateAttribute($object, $attribute) {
        ...
        if (!$this->validateValue($value)) {
            $message = $this->message !== null ? $this->message :
                Yii::t('yii', '{attribute} must be {type}.');
    ...
    public function validateValue($value) {
        ...
        if ($type === gettype($value))
            return true;
        elseif ($this->strict || is_array($value) || is_object($value) || is_resource($value)
            || is_bool($value)
        )
            return false;
        ...
        elseif ($type === 'double')
            return (boolean) preg_match('/^[-+]?([0-9]*\.)?[0-9]+([eE][-+]?[0-9]+)?$/',
                trim($value));
        elseif ($type === 'date')
            return CDateTimeParser::parse($value, $this->dateFormat,
                array ('month' => 1, 'day' => 1, 'hour' => 0, 'minute' => 0, 'second' => 0))
            !== false;
        elseif ($type === 'time')
            return CDateTimeParser::parse($value, $this->timeFormat) !== false;
        elseif ($type === 'datetime')
            return CDateTimeParser::parse($value, $this->datetimeFormat,
                array ('month' => 1, 'day' => 1, 'hour' => 0, 'minute' => 0, 'second' => 0))
            !== false;
